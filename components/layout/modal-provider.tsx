...
interface ModalOptions<T = any, K = any> {
  title?: string;
  content?: FC<T>;
  contentProps?: T;
  actions?: FC<K>;
...
interface ModalContextType<T = any> {
  show: <T = any>(options: ModalOptions<T>) => void;
  hide: () => void;
  updateProps: (newProps: Record<string, T>) => void;
}
...
export function ModalProvider({ children }: { children: ReactNode }) {
  ...
  const show: ModalContextType["show"] = (options: ModalOptions) => {
  ...
  const updateProps: ModalContextType["updateProps"] = (newProps) => {
        ...
        <>
          ...
          <div className={cn("bg-background border-white/10",
            ...
            <div className="flex items-center justify-between p-4 border-b">
              ...
              <Button size="icon" variant="ghost" onClick={hide} className="ml-auto">
                <X size={20} />
              </Button>
            </div>
